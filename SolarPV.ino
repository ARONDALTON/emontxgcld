...
void loop()
{
  ...
  if (rf12_recvDone())
  {
    if (rf12_crc == 0 && (rf12_hdr & RF12_HDR_CTL) == 0)  // and no rf errors
    {
      ...
      if (node_id == 5) {emontx = *(PayloadTX*) rf12_data; last_emontx = millis();}  //Assuming 5 is the emonTx NodeID
...
//    }
    ...
    //aron added
    usekwh += (((emontx.power1)+(emontx.power2))* 0.2) / 3600000;//*.5;
    genkwh += ((emontx.power3+(emontx.power4)) * 0.2) / 3600000;//*.5;
...
//    }
    ...
    //aron added
    cval_use = (emontx.power1)+(emontx.power2);
    cval_gen = emontx.power3+(emontx.power4);
